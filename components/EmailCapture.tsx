...
interface EmailCaptureProps {
  variant?: 'full' | 'banner';
  source?: 'checklist' | 'waitlist' | 'landing';
  buttonText?: string;
  placeholder?: string;
...
export default function EmailCapture({
  ...
  const resolvedButton = buttonText ?? (isWaitlist ? 'Request Access' : 'Get It Free');
  const resolvedSuccessHeading = successHeading ?? (isWaitlist ? "You're on the list!" : "You're on the list!");
  const resolvedSuccessBody = successBody ?? (isWaitlist
    ? "We'll reach out with early access details. Thanks for your interest."
    : "We'll send your free Estimating Checklist soon. Keep an eye on your inbox."
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (convexUrl) {
        ...
        await client.mutation(api.leads.subscribeEmail, { email, source });
      }
      gtagEvent('email_captured', { event_category: 'lead', event_label: source });
      setSubmitted(true);
    } catch {
      setSubmitted(true);
    } finally {
    ...
    return (
      <div className={variant === 'banner' ? 'bg-emerald-50 border-y border-emerald-200 py-4' : 'rounded-2xl p-8 md:p-12 border border-emerald-500/20 bg-emerald-500/5'}>
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h3 className="text-xl font-bold text-white mb-2">{resolvedSuccessHeading}</h3>
          <p className="text-slate-400">{resolvedSuccessBody}</p>
        </div>
      </div>
    );
  }
