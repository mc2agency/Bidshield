...
- 'lam': Liquid-Applied Membrane — use for any liquid-applied waterproofing membrane, whether conventional (insulation below) or IRMA/inverted (insulation above). Includes: cold-applied fluid membranes ("Cold Fluid Applied Waterproofing Membrane", "Cold Applied Liquid Membrane"), PMMA systems (Siplast Parapro®, Paracoat®, Terapro®), urethane liquid membranes. Also use for generic IRMA/PRMA/inverted assemblies where no manufacturer is named and no sheet membrane is identified. Labels: "Cold Fluid Applied Waterproofing Membrane on Base Sheet", "Liquid Membrane Cold-Applied", "IRMA Roofing Assembly", "Inverted Roof Membrane Assembly", "Green Roofing Assembly" (with fluid membrane), "PMMA", "Parapro", "Paracoat". CRITICAL: A cold fluid applied membrane on a concrete deck under XPS insulation with pavers/pedestals = 'lam' system, surface = 'pavers_pedestals'. A sloped topping concrete layer BELOW the membrane is substrate prep — list in layers, not as coverBoard. A base sheet layer BELOW the fluid membrane is a reinforcement/primer layer — list in layers. Typical cold-fluid IRMA assembly (deck to top): Cast-in-Place Concrete Deck → Sloped Topping Concrete → Base Sheet → Cold Fluid Applied Waterproofing Membrane → Drainage Mat → 7" XPS Insulation (R-35) → Filter Fabric → Pavers on Pedestals. Typical Parapro assembly (conventional, over metal deck): Metal Deck → Paratherm Polyiso → Coverboard → Pro Base TS → Pro Fleece → Parapro® PMMA Roof Membrane.
