...
Return only the JSON array. No explanation, no markdown fences.`;
    ...
    try {
          ...
          messages: [
            {
              ...
              content: [
                ...
                { type: "text", text: "Extract all requirements from this GC bid form document. Return only the JSON array." },
              ],
            },
          ],
        },
      ...
      if (apiErr?.name === "AbortError" || apiErr?.message?.includes("abort")) {
        return NextResponse.json({ error: "Analysis timed out — the GC bid form may be too long. Try uploading just the scope/exhibit pages." }, { status: 504 });
      }
      throw apiErr;
    } finally {
      clearTimeout(timeout);
    ...
    console.error("extract-gc-form error:", {
      name: err?.name,
      message: err?.message,
      status: err?.status,
