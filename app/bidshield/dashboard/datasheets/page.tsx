...
import { materialPrices } from "@/lib/bidshield/demo-data";
...
export default function DatasheetsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-xl font-semibold text-white">📑 Material Database & Data Sheets</h2>
      ...
      <div className="flex gap-3">
        <input
          ...
          className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        <button
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
              ...
              <tr>
                {["Product", "Price", "Unit", "Coverage", "Vendor", "Quote Date", "Data Sheet"].map(
                  (h) => (
                    <th
                      key={h}
                      className="text-left p-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-700 bg-slate-900"
                    >
                      {h}
                    </th>
                  )
            ...
            <tbody>
              {filtered.map((mat) => (
                <tr key={mat.product} className="border-b border-slate-700 hover:bg-slate-750">
      ...
      {uploadModal && (
        ...
        >
          ...
          >
            <h2 className="text-xl font-semibold text-white mb-2">Upload Datasheet</h2>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  type="text"
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="e.g., ProDiene 30 Cap FR"
                />
              </div>
              <div>
                <label className="block text-sm text-slate-300 mb-1">Category</label>
                <select className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-md text-sm">
                Upload
              </button>
            </div>
          </div>
