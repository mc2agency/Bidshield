      ...
      try {
        ...
        for (const item of np.v2ExtractionItems) {
          await createExtractionItem({
            ...
            originalExtractedText: item.originalExtractedText,
            extractedLayers: item.extractedLayers,
            normalizedLayerTokens: item.normalizedLayerTokens,
            archetypeId: item.archetypeId,
            archetypeVersion: item.archetypeVersion,
            confidence: item.confidence,
            needsReview: item.needsReview,
            classificationAudit: item.classificationAudit,
            sectionValues: item.sectionValues,
            requiredSectionsSnapshot: item.requiredSectionsSnapshot,
            optionalSectionsSnapshot: item.optionalSectionsSnapshot,
            hiddenSectionsSnapshot: item.hiddenSectionsSnapshot,
            defaultLayerOrderSnapshot: item.defaultLayerOrderSnapshot,
            legacySystemId: item.legacySystemId ?? undefined,
          });
        }
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {editingProject && <NewBidWizard isDemo={isDemo} isPro={isPro} onClose={() => setEditingProject(null)} onCreate={handleUpdateProject} editProject={{
        ...
        drawingDate: (editingProject as any).drawingDate,
        drawingRevision: (editingProject as any).drawingRevision,
