...
function getAddendumStatus(add: any): { label: string; color: string; icon: string } {
  if (add.affectsScope === true && add.repriced) {
    return { label: "Reviewed & Re-priced", color: "text-emerald-400", icon: "✓" };
  }
  if (add.affectsScope === false) {
    return { label: "Reviewed — No impact", color: "text-emerald-400", icon: "✓" };
  }
  if (add.affectsScope === true && !add.repriced) {
    return { label: "Pending Re-price", color: "text-red-400", icon: "⚠" };
  }
  return { label: "Not Reviewed", color: "text-amber-400", icon: "⚠" };
}
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Track all addenda and their impact on your bid</p>
        </div>
        {!isDemo && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700 text-center">
          ...
          <div className="text-[11px] text-slate-400">Total Addenda</div>
        </div>
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700 text-center">
          <div className={`text-2xl font-bold ${scopeAffecting > 0 ? "text-amber-400" : "text-slate-400"}`}>{scopeAffecting}</div>
          <div className="text-[11px] text-slate-400">Affect Scope</div>
        </div>
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700 text-center">
          <div className="text-2xl font-bold text-emerald-400">{repricedCount}</div>
          <div className="text-[11px] text-slate-400">Re-priced</div>
        </div>
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700 text-center">
          <div className={`text-2xl font-bold ${pendingRePrice > 0 ? "text-red-400" : "text-emerald-400"}`}>{pendingRePrice}</div>
          <div className="text-[11px] text-slate-400">Pending Re-price</div>
        </div>
      </div>
      ...
      {netPriceImpact !== 0 && (
        <div className="bg-slate-800 rounded-lg p-3 border border-slate-700 text-center">
          <span className="text-sm text-slate-400">Net Price Impact: </span>
          <span className={`text-sm font-bold ${netPriceImpact > 0 ? "text-red-400" : "text-emerald-400"}`}>
      ...
      {showAdd && (
        <div className="bg-slate-800 rounded-xl p-5 border border-emerald-500/50">
          <h3 className="text-sm font-semibold text-white mb-4">Add Addendum #{nextNumber}</h3>
          <div className="flex flex-col gap-4">
            <div>
              <label className="block text-xs text-slate-400 mb-1">Title / Description *</label>
              <input type="text" value={newAddendum.title} onChange={(e) => setNewAddendum({ ...newAddendum, title: e.target.value })} placeholder="Revised RTU schedule — 2 units upsized" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-slate-400 mb-1">Date Received</label>
                <input type="date" value={newAddendum.receivedDate} onChange={(e) => setNewAddendum({ ...newAddendum, receivedDate: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm" />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Priority</label>
                <select value={newAddendum.priority} onChange={(e) => setNewAddendum({ ...newAddendum, priority: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm">
            ...
            <div>
              ...
              <textarea value={newAddendum.notes} onChange={(e) => setNewAddendum({ ...newAddendum, notes: e.target.value })} placeholder="What does this addendum cover?" rows={2} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm resize-none" />
            </div>
            <div className="flex gap-3">
              <button onClick={handleAdd} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg transition-colors">Save</button>
              <button onClick={() => setShowAdd(false)} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm rounded-lg transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
...
function AddendumCard({
  ...
  return (
    <div className={`bg-slate-800 rounded-xl p-5 border border-slate-700 ${cardBorderColor(add)}`}>
      {/* Header Row */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap mb-1.5">
            <span className="text-xs font-bold bg-slate-700 text-slate-300 px-2 py-0.5 rounded">#{add.number}</span>
            {priority === "critical" && (
              <span className="text-[10px] font-bold bg-red-500/20 text-red-400 px-2 py-0.5 rounded uppercase">Critical</span>
            )}
            {priority === "high" && (
              <span className="text-[10px] font-bold bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded uppercase">High</span>
            )}
        ...
        {!isDemo && (
          <button onClick={() => onDelete(add._id)} className="text-slate-600 hover:text-red-400 text-xs transition-colors shrink-0">Delete</button>
        )}
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-700/50">
        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-400">Affects Roofing Scope?</span>
      ...
      {add.affectsScope === true && (
        <div className="mt-3 p-4 bg-slate-900/50 rounded-lg border border-slate-700/50">
          <div className="text-xs font-semibold text-slate-300 mb-3">Scope Impact</div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
            ...
            {!add.repriced && (
              <span className="text-[11px] font-semibold text-red-400">⚠ NEEDS RE-PRICING</span>
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                placeholder="0"
                className="w-32 bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-white text-sm"
                readOnly={isDemo}
              />
              {localPriceImpact && !isNaN(parseFloat(localPriceImpact)) && (
                <span className={`text-xs font-bold ${parseFloat(localPriceImpact) > 0 ? "text-red-400" : parseFloat(localPriceImpact) < 0 ? "text-emerald-400" : "text-slate-400"}`}>
      ...
      <div className="mt-3">
        ...
        <textarea
          ...
          rows={1}
          className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-white text-sm resize-none"
          readOnly={isDemo}
        />
      </div>
      ...
      <div className="mt-3 pt-3 border-t border-slate-700/50 flex items-center justify-between">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 text-xs font-medium transition-colors ${add.acknowledged ? "text-emerald-400" : "text-slate-500 hover:text-slate-300"}`}
        >
          <span className={`w-4 h-4 rounded border flex items-center justify-center text-[10px] ${add.acknowledged ? "bg-emerald-500/20 border-emerald-500" : "border-slate-600"}`}>{add.acknowledged ? "✓" : ""}</span>
          Acknowledged
        </button>
      </div>
    </div>
