  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-2">
        <div>
          <h2 className="text-xl font-semibold text-white">📨 RFIs — {project?.name || "Project"}</h2>
          <p className="text-sm text-slate-400">Track questions sent to GCs, architects, and engineers</p>
        </div>
        <button
        ...
        {(["all", "draft", "sent", "answered", "closed"] as const).map((s) => (
          <button
            ...
            className={`px-4 py-2 text-sm rounded-lg font-medium transition-colors ${
              filter === s ? "bg-emerald-600 text-white" : "bg-slate-800 text-slate-400 hover:text-white"
            }`}
          >
      ...
      {filteredRFIs.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {filteredRFIs.map((rfi: any) => {
            ...
            return (
              <div key={rfi._id as string} className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                <div
      ...
      {showCreateModal && (
        <div onClick={() => setShowCreateModal(false)} className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div onClick={(e) => e.stopPropagation()} className="bg-slate-800 rounded-2xl p-6 w-full max-w-lg border border-slate-700">
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <textarea value={newQuestion} onChange={(e) => setNewQuestion(e.target.value)} placeholder="What needs clarification?" rows={4} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
              <div>
                <label className="block text-sm text-slate-300 mb-1">Send To (email)</label>
                <input type="email" value={newSentTo} onChange={(e) => setNewSentTo(e.target.value)} placeholder="architect@example.com" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
            </div>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setShowCreateModal(false)} className="px-5 py-2.5 border border-slate-600 text-slate-400 rounded-lg text-sm">Cancel</button>
              <button onClick={handleCreate} disabled={!newQuestion.trim()} className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-lg text-sm transition-colors disabled:opacity-50">Create RFI</button>
            </div>
          </div>
        </div>
      )}
