...
export default function LaborTab({ isDemo, userId }: TabProps) {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        {!isDemo && (
          <div className="flex gap-2">
            <button onClick={handleSeedDefaults} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm rounded-lg transition-colors">Load Defaults</button>
            <button onClick={() => { setNewRate({ ...newRate, category: activeCategory }); setShowAdd(true); }} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg transition-colors">+ Add Rate</button>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {categories.map((cat) => {
          ...
          return (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm transition-all ${activeCategory === cat.id ? "bg-emerald-600 text-white" : "bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700"}`}>
              <span>{cat.icon}</span><span>{cat.label}</span>
              {count > 0 && <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeCategory === cat.id ? "bg-emerald-500" : "bg-slate-700"}`}>{count}</span>}
            </button>
          );
        })}
      </div>
        ...
        <div className="bg-slate-800 rounded-xl p-5 border border-emerald-500/50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={newRate.task} onChange={(e) => setNewRate({ ...newRate, task: e.target.value })} placeholder="TPO Install (mechanically attached)" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-slate-400 mb-1">Rate *</label>
                <input type="text" value={newRate.rate} onChange={(e) => setNewRate({ ...newRate, rate: e.target.value })} placeholder="450 SF" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm" />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Unit</label>
                <select value={newRate.unit} onChange={(e) => setNewRate({ ...newRate, unit: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm">
            ...
            <div>
              ...
              <input type="number" value={newRate.crew} onChange={(e) => setNewRate({ ...newRate, crew: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm" />
            </div>
            <div>
              <label className="block text-xs text-slate-400 mb-1">Notes</label>
              <input type="text" value={newRate.notes} onChange={(e) => setNewRate({ ...newRate, notes: e.target.value })} placeholder="Standard conditions" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm" />
            </div>
          </div>
          <div className="flex gap-3 mt-4">
            <button onClick={handleAdd} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg transition-colors">Save Rate</button>
      ...
      {filteredRates.length > 0 ? (
            ...
            <tbody>
              {filteredRates.map((rate: any) => (
                <tr key={rate._id} className="border-b border-slate-700/50 hover:bg-slate-750 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right"><span className="text-emerald-400 font-semibold">{rate.rate}</span><span className="text-slate-500 ml-1">{rate.unit}</span></td>
                  ...
                  {!isDemo && (
                    <td className="px-4 py-3"><button onClick={() => handleDelete(rate._id)} className="text-slate-500 hover:text-red-400 text-xs transition-colors">Delete</button></td>
                  )}
                </tr>
              ))}
            </tbody>
        ...
        <div className="text-center py-16 bg-slate-800 rounded-xl border border-slate-700">
          ...
          <p className="text-sm text-slate-400 mb-4">No rates in {categories.find(c => c.id === activeCategory)?.label || activeCategory} yet</p>
          {!isDemo && (
            <div className="flex gap-3 justify-center">
              <button onClick={handleSeedDefaults} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm rounded-lg transition-colors">Load Industry Defaults</button>
              <button onClick={() => { setNewRate({ ...newRate, category: activeCategory }); setShowAdd(true); }} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm rounded-lg transition-colors">Add Custom Rate</button>
            </div>
          )}
        </div>
      )}
