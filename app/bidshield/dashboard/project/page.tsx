    ...
    <div className="-m-6 flex flex-col" style={{ minHeight: "calc(100vh - 4rem)" }}>
      ...
      <div className="bg-white border-b border-slate-200 px-5 py-2.5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-1.5 text-sm min-w-0">
          ...
          <span
            className={`font-medium truncate max-w-[160px] ${activeTab ? "text-slate-500 cursor-pointer hover:text-slate-700" : "text-slate-800"}`}
          ...
          {activeTab && (
            <>
              <span className="text-slate-200 shrink-0">/</span>
              <span className="text-slate-900 font-semibold truncate">{activeTabLabel}</span>
            </>
          )}
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Panel A — dark left sidebar */}
        <aside
          className="hidden lg:flex flex-col shrink-0 border-r border-white/10 overflow-y-auto"
          style={{ width: 220, background: "#0f1117", minHeight: "calc(100vh - 8rem)" }}
        >
          {/* Project info */}
          <div className="px-4 pt-4 pb-3 border-b border-white/8">
            <div className="text-white font-semibold text-[13px] leading-snug truncate">{projectData?.name}</div>
            <div className="text-slate-400 text-[11px] mt-0.5 truncate">{projectData?.location}</div>
            {(projectData as any)?.gc && (
              <div className="text-slate-500 text-[11px] truncate">{(projectData as any).gc}</div>
            )}
            {/* Readiness bar */}
                ...
                >
                  ...
                  {sectionScore !== undefined && (
                    <span
                      ...
                      style={{
                        color: sectionScore >= 80 ? "#10b981" : sectionScore >= 50 ? "#f59e0b" : "#ef4444",
                      }}
                    >
                      {sectionScore}%
                    </span>
        ...
        <main className="flex-1 bg-white overflow-auto min-w-0">
          {activeTab ? (
            <>
              {/* Tab header */}
              <div className="px-6 py-3 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white z-10">
        ...
        >
          ...
          <div className="grid grid-cols-3 border-b border-slate-200">
            ...
            <div className="px-3 py-2.5 border-r border-slate-200">
              <div className="text-[12px] font-bold text-slate-800">{bidAmt ? `$${(bidAmt / 1000).toFixed(0)}K` : "—"}</div>
          ...
          {sys && (
            <div className="px-4 py-3 flex flex-col gap-3 border-b border-slate-200">
              <div>
                ...
                <div className="text-[12px] text-slate-700">{sys.seamMethod.split("(")[0].trim()}</div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {sys.manufacturers.slice(0, 4).map(m => (
                    <span key={m} className="text-[10px] bg-white border border-slate-200 text-slate-600 px-2 py-0.5 rounded-full">{m}</span>
                  ))}
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="text-[12px] text-slate-700">{sys.thicknessOptions.join(" · ")}</div>
              </div>
              <button
          ...
          {(projectData as any)?.notes && (
            <div className="px-4 py-3 border-b border-slate-200">
              ...
              <p className="text-[11px] text-slate-600 leading-relaxed">{(projectData as any).notes}</p>
            </div>
          )}
